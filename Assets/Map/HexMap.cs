...
{
    public static HexMap sSingleton;
    ...
    private void MakeMap_New()
    {
        ...
        for (int i = 0; i < map_size_.x; i++)
        {
            for (int j = 0; j < map_size_.y; j++)
            {
                tiles_[i][j] = children_[counter_];
                counter_++;
                tiles_[i][j].position_ = new Vector2Int(i, j);
                AddUITile(tiles_[i][j]);
            }
        }
    }
        ...
        for (int i = 0; i < map_size_.x; i++)
        {
            for (int j = 0; j < map_size_.y; j++)
            {
                ...
                if (i == 0)
                {
                    ...
                    {
                        if ((int)children_[k].transform.position.z == (int)tiles_[i][j - 1].transform.position.z)
                        {
                            if ((int)children_[k].transform.position.x <= (int)tiles_[i][j - 1].transform.position.x + tile_size_)
                            {
                                Debug.LogError(j);
                                Debug.LogError(children_[k], children_[k].gameObject);
    ...
    {
        var dirs = new HexDirection[] { HexDirection.DownLeft, HexDirection.DownRight, HexDirection.Left, HexDirection.Right, HexDirection.UpLeft, HexDirection.UpRight };
    ...
    public TileBase GetNeightborTile(TileBase tile, HexDirection direction)
    {
        switch (direction)
        {
            case HexDirection.DownLeft: return GetDownLeftNeighbor(tile);
            case HexDirection.DownRight: return GetDownRightNeighbor(tile);
            case HexDirection.Left: return GetLeftNeighbor(tile);
            case HexDirection.Right: return GetRightNeighbor(tile);
            case HexDirection.UpLeft: return GetUpLeftNeighbor(tile);
            case HexDirection.UpRight: return GetUpRightNeighbor(tile);
            default: Debug.LogError("wtf direction"); return GetDownRightNeighbor(tile);
        }
    }
    ...
    private TileBase GetUpLeftNeighbor(TileBase tile)
    {
        int x = tile.position_.x, y = tile.position_.y;
    ...
    private TileBase GetUpRightNeighbor(TileBase tile)
    {
        int x = tile.position_.x, y = tile.position_.y;
        if (x > 0 && y < tiles_[x].Length - 1 && IsAcceptableTile(tiles_[x - 1][y].type_))
            return tiles_[x - 1][y + 1];
        return null;
    }

    private TileBase GetDownLeftNeighbor(TileBase tile)
    {
        int x = tile.position_.x, y = tile.position_.y;
    ...
    private TileBase GetDownRightNeighbor(TileBase tile)
    {
        int x = tile.position_.x, y = tile.position_.y;
        if (x < tiles_[x].Length - 1 && y > 0 && IsAcceptableTile(tiles_[x + 1][y].type_))
            return tiles_[x + 1][y];
        return null;
    }
